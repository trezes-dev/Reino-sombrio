...
def recarregar_mapa():
    global MAPA_CACHE
    c = con()
    MAPA_CACHE = {row[0]: row[1] for row in c.execute("SELECT (x || ',' || y), tipo FROM mapa_tiles").fetchall()}
    # Precisa ser tupla (x,y)
    MAPA_CACHE = {}
    for x, y, tipo in c.execute("SELECT x, y, tipo FROM mapa_tiles").fetchall():
        MAPA_CACHE[(x, y)] = tipo
    c.close()
    recarregar_objetos()
    print("Mapa recarregado: " + str(len(MAPA_CACHE)) + " tiles")

def recarregar_objetos():
    global OBJETOS_CACHE
    c = con()
    OBJETOS_CACHE = {(r[0], r[1]): r[2] for r in c.execute("SELECT x, y, subtipo FROM mapa_objetos").fetchall()}
    c.close()

def tile_em(x, y):
    """Le do cache em memoria — zero queries SQLite."""
    t = MAPA_CACHE.get((x, y))
    if t is None:
        t = MAPA_CACHE.get(str(x) + "," + str(y))
    return t if t is not None else "agua"


def bloqueia(x, y):
    """True se o tile esta bloqueado. Le do cache — zero queries."""
    tipo_chao = tile_em(x, y)
    if tipo_chao in ("agua", "vazio", "muro_topo", "muro_vertical", "muro2_topo", "muro2_vertical", "muro_castelo", "muro_canto"):
        return True
    subtipo = OBJETOS_CACHE.get((x, y))
...
def api_objetos():
    c = con()
    cursor = c.execute("SELECT x, y, tipo, subtipo FROM mapa_objetos")
    objetos = [{"x": r[0], "y": r[1], "tipo": r[2], "subtipo": r[3]} for r in cursor.fetchall()]
    c.close()
...
SLIME_SPAWNS = [
    # Q1 Sup Esq (grama_escura) -> Azul
    ("Slime Azul", 30, 30), ("Slime Azul", 50, 50), ("Slime Azul", 25, 70),
    # Q2 Sup Dir (grama_normal) -> Verde
    ("Slime Verde", 150, 30), ("Slime Verde", 170, 50), ("Slime Verde", 140, 60),
    # Q4 Inf Dir (pedra_ruinas) -> Vermelho
    ("Slime Vermelho", 150, 150), ("Slime Vermelho", 170, 140), ("Slime Vermelho", 140, 165),
]

def spawn_slimes_se_faltando():
    c = con()
    n = c.execute("SELECT COUNT(*) FROM monstros").fetchone()[0]
    if n < len(SLIME_SPAWNS):
        for nome, x, y in SLIME_SPAWNS:
            existe = c.execute("SELECT id FROM monstros WHERE nome=? AND x=? AND y=?", (nome, x, y)).fetchone()
            if not existe:
                c.execute("INSERT INTO monstros (nome, x, y, hp, dano) VALUES (?,?,?,?,?)", (nome, x, y, 50, 5))
        c.commit()
    c.close()

def ia_monstros_DESATIVADA():
    while True:
        time.sleep(3)
        try:
            # spawn_slimes_se_faltando()
            c = con()
